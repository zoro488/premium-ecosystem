...
const COLORS = {
  ...
  heatmap: ['#1E3A8A', '#3B82F6', '#60A5FA', '#93C5FD', '#DBEAFE', '#FEF3C7', '#FDE047', '#FACC15', '#F59E0B', '#DC2626'],
  success: '#10B981',
  warning: '#F59E0B',
  danger: '#EF4444',
  info: '#3B82F6'
            ...
            <defs>
              {days.map((day, idx) => (
                <linearGradient key={day} id={`color${day}`} x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor={COLORS.primary[idx]} stopOpacity={0.8}/>
                  <stop offset="95%" stopColor={COLORS.primary[idx]} stopOpacity={0.1}/>
                </linearGradient>
              ))}
            </defs>
    ...
    >
      ...
      <div className="relative inline-flex items-center justify-center">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          <p className="text-sm text-slate-400 mt-1">{value.toLocaleString()} / {max.toLocaleString()}</p>
        </div>
      </div>
...
export const ConversionFunnel = ({ data, width = '100%', height = 400 }) => {
  const funnelData = data.map((item, idx) => ({
    ...item,
    fill: COLORS.gradient[idx % COLORS.gradient.length]
  }));
...
export const PeriodComparison = ({ currentData, previousData, metricName, width = '100%', height = 350 }) => {
  const comparisonData = currentData.map((item, idx) => ({
    name: item.name || item.mes || item.fecha,
    actual: item.value || item.total || 0,
    anterior: previousData[idx]?.value || previousData[idx]?.total || 0
  }));
    ...
    >
      ...
      <div className="mt-4 p-4 bg-yellow-500/10 rounded-xl border border-yellow-500/20">
        <p className="text-sm text-yellow-400 flex items-center gap-2">
          ...
          Las proyecciones están basadas en tendencias históricas y pueden variar según condiciones del mercado.
        </p>
      </div>
    </motion.div>
  );
