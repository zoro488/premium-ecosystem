...
function initInteractions() {
    ...
    emailInput.addEventListener('input', () => {
        ...
        if (emailInput.value.length > 5 && emailInput.value.includes('@')) {
            ...
            analysisIndicator.innerHTML = '<span class="w-2 h-2 bg-white/50 rounded-full animate-pulse mr-2"></span>Analizando...';
            
            // Simulamos análisis rápido de Gemini
            analysisTimeout = setTimeout(async () => {
                let isSafe = true; // Por defecto
                ...
                // const result = await checkDomainWithGemini(emailInput.value);
                 ...
                 if (emailInput.value.includes('chronos.ai')) {
                     analysisIndicator.innerHTML = '<span class="text-green-400 flex items-center"><svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg> Dominio Seguro</span>';
                 } else {
...
async function handleLogin(e) {
    ...
    try {
        ...
        showModal(isAiMsg ? 'Enlace Chronos Establecido' : 'Acceso Exitoso', welcomeMsg, isAiMsg ? 'ai' : 'success');
        if (isAiMsg) confetti({ particleCount: 60, spread: 70, origin: { y: 0.7 }, colors: ['#ffffff', '#a0a0a0'], disableForReducedMotion: true });
...
function showModal(title, body, type) {
    ...
    const icons = {
        success: '<svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>',
        error: '<svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" /></svg>',
