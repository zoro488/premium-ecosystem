...
export default function PerfilCliente({
  ...
  return (
    <>
      ...
      >
        <motion.div
          className="relative w-full max-w-7xl max-h-[95vh] overflow-hidden bg-gradient-to-br from-slate-900 via-slate-900 to-zinc-800/20 rounded-2xl border border-white/10 shadow-2xl"
          initial={{ scale: 0.9, y: 20 }}
          animate={{ scale: 1, y: 0 }}
          exit={{ scale: 0.9, y: 20 }}
        >
          {/* Header */}
          <div className="bg-gradient-to-r from-zinc-800 to-zinc-700 p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className={`backdrop-blur-sm rounded-lg p-4 border ${
                tieneAdeudo
                  ? 'bg-zinc-9000/20 border-zinc-500/30'
                  : 'bg-zinc-9000/20 border-zinc-500/30'
              }`}>
                <div className="flex items-center gap-2 mb-2">
                  <Wallet className={tieneAdeudo ? 'text-zinc-200' : 'text-zinc-200'} size={16} />
                  <span className="text-white/60 text-xs">Adeudo Actual</span>
                </div>
                <p className={`text-2xl font-bold ${tieneAdeudo ? 'text-zinc-200' : 'text-zinc-200'}`}>
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                {tabActual === 'ventas' && (
                  <motion.div
                    layoutId="activeTab"
                    className="absolute bottom-0 left-0 right-0 h-0.5 bg-zinc-800"
                  />
                )}
              </button>
                  ...
                  >
                    {ventas.length === 0 ? (
                      ...
                      ventas.map((venta, idx) => (
                        <motion.div
                          key={venta.id || idx}
                          className="bg-slate-800/50 border border-white/10 rounded-lg p-4 hover:border-zinc-800/30 transition-all"
                      ...
                      abonos.map((abono, idx) => (
                        ...
                        >
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <div className="flex items-center gap-3 mb-2">
                                <CheckCircle className="text-zinc-200" size={20} />
                                <span className="text-white font-semibold">
                                  Abono #{abono.id || idx + 1}
                                </span>
                                <span className="px-2 py-1 rounded text-xs bg-zinc-800/20 text-zinc-300">
                                  {abono.metodoPago}
                                </span>
                              </div>
                              <div className="text-sm text-white/60 space-y-1">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-slate-800/50 border border-white/10 rounded-lg p-6">
                        <div className="flex items-center gap-2 mb-4">
                          <CreditCard className="text-zinc-800" size={20} />
            ...
            <div className="lg:col-span-1">
              <div className="bg-gradient-to-br from-slate-800/80 to-slate-900/80 border border-white/10 rounded-lg p-6 sticky top-0">
                ...
                <div className={`p-4 rounded-lg mb-6 ${
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    {tieneAdeudo ? (
                      <AlertCircle className="text-zinc-200" size={20} />
                    ) : (
                      <CheckCircle className="text-zinc-200" size={20} />
                    )}
                  ...
                  <p className={`text-3xl font-bold ${
                    tieneAdeudo ? 'text-zinc-200' : 'text-zinc-200'
                  }`}>
                    ${stats.adeudoActual.toLocaleString('es-MX', { minimumFractionDigits: 2 })}
                  </p>
                </div>
                ...
                {tieneAdeudo && (
                  <button
                    ...
                    className="w-full px-4 py-3 bg-gradient-to-r from-zinc-800 to-zinc-700 hover:from-zinc-800 hover:to-zinc-700 rounded-lg text-white font-semibold transition-all transform hover:scale-105 flex items-center justify-center gap-2"
                  >
                    <DollarSign size={20} />
                    Registrar Abono
                  </button>
                ...
                {!tieneAdeudo && (
                  <div className="text-center py-4">
                    <CheckCircle className="text-zinc-200 mx-auto mb-2" size={32} />
                    <p className="text-zinc-200 font-semibold">¡Sin Adeudos!</p>
