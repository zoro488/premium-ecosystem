...
export const KeyboardShortcutsHelp = ({ isOpen, onClose }) => {
  ...
  const categories = {
    'Búsqueda y Navegación': ['SEARCH', 'DASHBOARD', 'ORDERS', 'DISTRIBUTORS', 'WAREHOUSE', 'SALES', 'CLIENTS', 'BANKS', 'REPORTS'],
            ...
            <div key={category}>
              ...
              <div className="space-y-2">
                {shortcuts.map(shortcut => {
                  ...
                  return (
                    <div key={shortcut} className="flex items-center justify-between p-3 bg-white/5 rounded-lg hover:bg-white/10 transition-colors">
                      <span className="text-slate-300">{def.description}</span>
                      <div className="flex gap-2">
                        {def.keys.map((key, idx) => (
                          <kbd
                            key={idx}
