...
export const useBulkActions = (onSuccess, onError) => {
  ...
  const executeBulkAction = useCallback(async (items, action, options = {}) => {
  ...
  const bulkUpdate = useCallback(async (items, updateFunction, updates) => {
    return executeBulkAction(
      items,
      (item) => updateFunction(item, updates),
      { stopOnError: false }
    );
  }, [executeBulkAction]);

  // Exportar múltiples items
  const bulkExport = useCallback(async (items, exportFunction) => {
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold">
              {selectedCount === totalCount ? 'Todos' : selectedCount} seleccionado{selectedCount !== 1 ? 's' : ''}
            </span>
          </div>
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[9999] p-4">
      <div className="glass rounded-2xl p-8 max-w-md w-full border border-white/10 animate-scale-in">
        <div className="text-center">
          <div className={`w-16 h-16 mx-auto mb-4 rounded-full bg-gradient-to-r ${colorClasses[confirmColor]} flex items-center justify-center`}>
          ...
          {itemCount && (
            <p className="text-sm text-slate-400 mb-6">
              Se procesarán <span className="font-bold text-purple-400">{itemCount}</span> elemento{itemCount !== 1 ? 's' : ''}
            </p>
          )}
