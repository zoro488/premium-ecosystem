/**
 * 🏦 PANEL GYA - GASTOS Y ABONOS (FIREBASE CONNECTED)
 * ==================================================
 * Panel conectado a Firestore colección 'gya'
 * 306 registros cargados desde Firebase
 */
...
export default function PanelGYA() {
  // 🔥 CONEXIÓN A FIREBASE (306 registros)
  const [gastosAbonosFirebase, setGastosAbonosFirebase] = useState([]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        setGastosAbonosFirebase(datos);
        setLoading(false);
      },
      (err) => {
        console.error('Error cargando GYA:', err);
  ...
  // ⚡ USAR DATOS DE FIREBASE EN LUGAR DE STORE
  const movimientos = useMemo(() => {
    return gastosAbonosFirebase.map((item) => ({
      fecha: item.fecha,
      tipo: item.tipo === 'gasto' ? 'Gasto' : 'Abono',
      origen: item.origen,
      destino: item.destino,
      valor: parseFloat(item.valor) || 0,
      concepto: item.concepto || '',
      pesos: parseFloat(item.pesos) || 0,
      observaciones: item.observaciones || '',
      tc: item.tc || 0,
      panel: 'GYA Firebase',
    }));
  }, [gastosAbonosFirebase]);

  // Ocultar bancos legacy (comentado por ahora)
  const _legacyMovimientos = useMemo(() => {
    const allMovimientos = [];
    /* LEGACY CODE - Comentado para usar Firebase
      ...
      <div className="relative min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
        <div className="flex items-center justify-center h-screen">
          <div className="text-center">
            ...
            <p className="text-white text-lg">Cargando datos GYA desde Firebase...</p>
            <p className="text-slate-400 text-sm mt-2">306 registros</p>
          </div>
        </div>
      </div>
    );
  ...
  if (error) {
    return (
      <div className="relative min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-8">
        <div className="bg-red-500/20 border border-red-500 text-red-200 px-6 py-4 rounded-lg max-w-2xl mx-auto mt-20">
